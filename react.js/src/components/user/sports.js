...
const createStyles = {
  ...
  export class guest extends Component {
    ...
    componentDidMount(){
        axios({
            method :'GET',
            url : 'http://localhost:9000/userget/sports',
    ...
    handlebookings=(id)=>{
        this.props.history.push(`/singlesports/?id=${id}`);
    }
    render() {
        ...
        return (
            <div>
                <div class="admin-classroom">
                    <h1 class="admin-item-block admin-button" >sports</h1>
                    <div class="scroll">
                        {rooms.map((item)=>{
                            return <div class="admin-class-items">
                                        <div class="admin-class-sub-items">
                                            <span class="admin-obj-font admin-item-block">sport name:{item.name}</span>
                                            <span class="admin-obj-font admin-item-block">court:{item.court}</span>
