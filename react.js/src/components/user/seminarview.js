...
const createStyles = {
  ...
  export class seminar extends Component {
    ...
    componentDidMount(){
        axios({
            method :'GET',
            url : 'http://localhost:9000/userget/seminarhall',
    ...
    handlebookings=(id)=>{
        this.props.history.push(`/singlehall/?id=${id}`);
    }
    render() {
        ...
        return (
            <div>
                <div class="admin-classroom">
                    <h1 class="admin-item-block admin-button"  >seminar hall</h1>
                    <div class="scroll">
                        {rooms.map((item)=>{
                            return <div class="admin-class-items">
                                        <div class="admin-class-sub-items">
                                            <span class="admin-obj-font admin-item-block">hall No:{item.hall}</span>
